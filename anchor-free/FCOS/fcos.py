...
class FCOSHead(nn.Module):
    ...
    def compute_loss(
        ...
        gt_classes_targets[foregroud_mask, all_gt_classes_targets[foregroud_mask]] = 1.0  # 类别编码
...
class FCOS(nn.Module):
    ...
    def compute_loss(
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            x, y = anchor_centers.unsqueeze(dim=2).unbind(dim=1)  # [N, 1], [N, 1] 拆分坐标
            x0, y0, x1, y1 = gt_boxes.unsqueeze(dim=0).unbind(dim=2)  # [1, M], [1, M], [1, M], [1, M]
            pairwise_dist = torch.stack([x - x0, y - y0, x1 - x, y1 - y], dim=2)  # [N, M, 4] 计算ltrb

            # anchor point must be inside gt
            pairwise_match &= pairwise_dist.min(dim=2).values > 0  # ltrb最小值大于0

            # each anchor is only responsible for certain scale range. 0~64, 64~128, 128~256, 256~512, 512~INF
            lower_bound = anchor_sizes * 4
            lower_bound[: num_anchors_per_level[0]] = 0
            upper_bound = anchor_sizes * 8
            upper_bound[-num_anchors_per_level[-1]:] = float("inf")
            pairwise_dist = pairwise_dist.max(dim=2).values  # max(l, t, r, b)
            pairwise_match &= (pairwise_dist > lower_bound[:, None]) & (pairwise_dist < upper_bound[:, None])

            # match the GT box with minimum area, if there are multiple GT matches
            gt_areas = (gt_boxes[:, 2] - gt_boxes[:, 0]) * (gt_boxes[:, 3] - gt_boxes[:, 1])  # [N, ]
            pairwise_match = pairwise_match.to(torch.float32) * (1e8 - gt_areas[None, :])
            min_values, matched_idx = pairwise_match.max(dim=1)  # R, per-anchor match 当一个先验框与多个真实框匹配时, 只负责面积小的真实框
