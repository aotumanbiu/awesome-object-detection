...
def quality_focal_loss(pred, target, beta=2.0):
    ...
    # 负样本由0分数监督
    pred_sigmoid = pred.sigmoid()  # [N, num_classes]
    scale_factor = pred_sigmoid  # [N, num_classes]
    zerolabel = scale_factor.new_zeros(pred.shape)  # [N, num_classes]
    # 这里默认是目标全为背景时的损失
    loss = F.binary_cross_entropy_with_logits(pred, zerolabel, reduction='none') * scale_factor.pow(beta)

    # FG cat_id: [0, num_classes -1], BG cat_id: num_classes
    bg_class_ind = pred.size(1)  # num_classes
    pos = ((label >= 0) & (label < bg_class_ind)).nonzero().squeeze(1)  # 正样本标签的索引
    pos_label = label[pos].long()  # 取出正样本索引对应的类别预测
