...
class AnchorGenerator:
    ...
    def grid_priors(self, featmap_sizes, dtype=torch.float32, device='cuda'):
        ...
        """
        assert self.num_levels == len(featmap_sizes)  # 最前面已经判断过一次 这里再次确认
        multi_level_anchors = []
        for i in range(self.num_levels):
            anchors = self.single_level_grid_priors(
                featmap_sizes[i], level_idx=i, dtype=dtype, device=device)
            multi_level_anchors.append(anchors)
        return multi_level_anchors  # 返回不同尺寸特征图所对应的先验框坐标
    ...
    def single_level_grid_priors(self,
        ...
        base_anchors = self.base_anchors[level_idx].to(device).to(dtype)  # 获得某尺寸特征图先验框的基础大小
        feat_h, feat_w = featmap_size  # 特征图高，宽
        stride_w, stride_h = self.strides[level_idx]  # 当前特征图对应的下采样倍数
        # First create Range with the default dtype, than convert to
        # target `dtype` for onnx exporting.
        # 获得相对原图大小的中心坐标 shift_x, shift_y
        shift_x = torch.arange(0, feat_w, device=device).to(dtype) * stride_w
        shift_y = torch.arange(0, feat_h, device=device).to(dtype) * stride_h

        shift_xx, shift_yy = self._meshgrid(shift_x, shift_y)  # 中心点所组成网格的横纵坐标
        shifts = torch.stack([shift_xx, shift_yy, shift_xx, shift_yy], dim=-1)
        # first feat_w elements correspond to the first row of shifts
        # add A anchors (1, A, 4) to K shifts (K, 1, 4) to get
        # shifted anchors (K, A, 4), reshape to (K*A, 4)

        all_anchors = base_anchors[None, :, :] + shifts[:, None, :]  # 获得左上和右下角坐标(x1, y1, x2, y2)
    ...
    def valid_flags(self, featmap_sizes, pad_shape, device='cuda'):
        ...
        multi_level_flags = []
        for i in range(self.num_levels):
            anchor_stride = self.strides[i]  # 特征图下采样倍数 (8, 8) (16 ,16) (32, 32) (64, 64) (128, 128)
            feat_h, feat_w = featmap_sizes[i]  # 特征图长宽
            h, w = pad_shape[:2]  # ori_shape(500, 375) -> img_shape(1067, 800) -> pad_shape(1088, 800)
            valid_feat_h = min(int(np.ceil(h / anchor_stride[1])), feat_h)  # 防止越界
