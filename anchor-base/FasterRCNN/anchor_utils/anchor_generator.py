...
class AnchorGenerator(nn.Module):
    ...
    def forward(self, image_list: ImageList, feature_maps: List[Tensor]) -> List[Tensor]:
        grid_sizes = [feature_map.shape[-2:] for feature_map in feature_maps]  # FPN输出特征的尺寸 [38, 25], [38, 25], [19, 13]
        image_size = image_list.tensors.shape[-2:]  # 输入图像大小[1216, 800]
        dtype, device = feature_maps[0].dtype, feature_maps[0].device
        strides = [  # 特征图相对输入图像的下采样倍数
            [
        ...
        anchors_over_all_feature_maps = self.grid_anchors(grid_sizes, strides)  # FPN每项输出特征对应的先验框数量
        anchors: List[List[torch.Tensor]] = []
        for _ in range(len(image_list.image_sizes)):
            anchors_in_image = [anchors_per_feature_map for anchors_per_feature_map in anchors_over_all_feature_maps]
            anchors.append(anchors_in_image)
        anchors = [torch.cat(anchors_per_image) for anchors_per_image in anchors]  # B x [32205, 4](每个图像中的所有先验)
        return anchors
