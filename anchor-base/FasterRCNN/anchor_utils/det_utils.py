...
class BalancedPositiveNegativeSampler:
    ...
    def __call__(self, matched_idxs: List[Tensor]) -> Tuple[List[Tensor], List[Tensor]]:
        ...
        for matched_idxs_per_image in matched_idxs:
            ...
            # randomly select positive and negative examples 先打乱后选取
            perm1 = torch.randperm(positive.numel(), device=positive.device)[:num_pos]
            perm2 = torch.randperm(negative.numel(), device=negative.device)[:num_neg]

            pos_idx_per_image = positive[perm1]  # 正样本
            neg_idx_per_image = negative[perm2]  # 负样本
...
class BoxCoder:
    ...
    def decode(self, rel_codes: Tensor, boxes: List[Tensor]) -> Tensor:
        ...
        boxes_per_image = [b.size(0) for b in boxes]  # 每张图中所有先验框的数量
        concat_boxes = torch.cat(boxes, dim=0)  # 所有先验框
        box_sum = 0
        for val in boxes_per_image:
            box_sum += val
        if box_sum > 0:
            rel_codes = rel_codes.reshape(box_sum, -1)
        pred_boxes = self.decode_single(rel_codes, concat_boxes)  # 先验框解码生成预测框
        if box_sum > 0:
...
class Matcher:
    ...
    def __call__(self, match_quality_matrix: Tensor) -> Tensor:
        ...
        matched_vals, matches = match_quality_matrix.max(dim=0)  # 与每个先验框最为匹配真实框的IoU, 真实框的索引
    ...
    def set_low_quality_matches_(self, matches: Tensor, all_matches: Tensor, match_quality_matrix: Tensor) -> None:
        ...
        highest_quality_foreach_gt, _ = match_quality_matrix.max(dim=1)  # 与真实框最为对应先验框的IOU和索引
        # Find highest quality match available, even if it is low, including ties
        # 每一个真实框都需要找到一个最佳匹配度的预测框, 可能其IOU低于0.7, 也视为正样本
