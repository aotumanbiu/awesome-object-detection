...
class RoIHeads(nn.Module):
    ...
    def subsample(self, labels):
        ...
        for img_idx, (pos_inds_img, neg_inds_img) in enumerate(zip(sampled_pos_inds, sampled_neg_inds)):
            img_sampled_inds = torch.where(pos_inds_img | neg_inds_img)[0]  # 取出筛选后的512个样本(正样本+负样本)
    ...
    def select_training_samples(
        ...
        gt_boxes = [t["boxes"].to(dtype) for t in targets]  # 真实框信息
        gt_labels = [t["labels"] for t in targets]  # 真实框对应类别标签

        # append ground-truth bboxes to propos 把真实框添加到建议框中
        proposals = self.add_gt_proposals(proposals, gt_boxes)

        # get matching gt indices for each proposal 先验对应的真实框索引, 真实框对应类别
        matched_idxs, labels = self.assign_targets_to_proposals(proposals, gt_boxes, gt_labels)
        # sample a fixed proportion of positive-negative proposals 正负总数为512 x BS
        ...
        regression_targets = self.box_coder.encode(matched_gt_boxes, proposals)  # 这里是吧RPN的生成建议框(预测框)作为Head的先验框
    ...
    def forward(
        ...
        if self.training:
            # 建议框, 建议框匹配真实框索引, 建议框匹配真实框类别, 建议框与其对应先验框的编码
        ...
        box_features = self.box_roi_pool(features, proposals, image_shapes)  # in(FPN输出, 建议框) -> out[1024, 256, 7, 7]
        box_features = self.box_head(box_features)  # [1024, 1024]
        class_logits, box_regression = self.box_predictor(box_features)  # [1024, 91], [1024, 364]
...
"""
https://zhuanlan.zhihu.com/p/390734942
https://www.cnblogs.com/shuimuqingyang/p/14356218.html
"""