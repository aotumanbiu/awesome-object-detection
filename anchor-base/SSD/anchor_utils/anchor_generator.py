...
class DefaultBoxGenerator(nn.Module):
    ...
    def __init__(
        ...
        if scales is None:
            ...
            self.scales = scales  # scales指的是SSD每层应负责所预测物体的尺度范围
    ...
    def _generate_wh_pairs(
        ...
        for k in range(num_outputs):  # num_outputs指负责预测的特征图数量
            # Adding the 2 default width-height pairs for aspect ratio 1 and scale s'k
            s_k = self.scales[k]
            s_prime_k = math.sqrt(self.scales[k] * self.scales[k + 1])
            wh_pairs = [[s_k, s_k], [s_prime_k, s_prime_k]]  # 一个最小的正方形和一个最大的正方形 其长宽比都为一

            # Adding 2 pairs for each aspect ratio of the feature map k
            for ar in self.aspect_ratios[k]:  # 添加长方形
                sq_ar = math.sqrt(ar)
    ...
    def _grid_default_boxes(
        ...
        for k, f_k in enumerate(grid_sizes):
            # Now add the default boxes for each width-height pair 现在为每个宽高对添加默认框
            if self.steps is not None:    # step设置为特征图每个网格对应图像的像素点数
                x_f_k = image_size[0] / self.steps[k]
                y_f_k = image_size[1] / self.steps[k]
            else:
                y_f_k, x_f_k = f_k

            shifts_x = ((torch.arange(0, f_k[1]) + 0.5) / x_f_k).to(dtype=dtype)  # 先求出网格中心点 x 然后 归一化消除量纲
            shifts_y = ((torch.arange(0, f_k[0]) + 0.5) / y_f_k).to(dtype=dtype)  # 先求出网格中心点 y 然后 归一化消除量纲
            shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x, indexing="ij")  # 所有点的横纵坐标
            shift_x = shift_x.reshape(-1)
            shift_y = shift_y.reshape(-1)

            shifts = torch.stack((shift_x, shift_y) * len(self._wh_pairs[k]), dim=-1).reshape(-1, 2)  # 乘len是因为当前一个点负责len个先验框的预测
            ...
            wh_pairs = _wh_pair.repeat((f_k[0] * f_k[1]), 1)  # 使其与上面shifts对齐

            default_box = torch.cat((shifts, wh_pairs), dim=1)  # (cx, cy, w, h)
    ...
    def forward(self, image_list: ImageList, feature_maps: List[Tensor]) -> List[Tensor]:
        grid_sizes = [feature_map.shape[-2:] for feature_map in feature_maps]  # 计算每层特征的尺度
        image_size = image_list.tensors.shape[-2:]
        dtype, device = feature_maps[0].dtype, feature_maps[0].device
        ...
        for _ in image_list.image_sizes:
            ...
            dboxes_in_image = torch.cat(
                [
                    (dboxes_in_image[:, :2] - 0.5 * dboxes_in_image[:, 2:]) * x_y_size,  # 左上角坐标 * 300 得到绝对大小
                    (dboxes_in_image[:, :2] + 0.5 * dboxes_in_image[:, 2:]) * x_y_size,  # 右下角坐标 * 300 得到绝对大小
                ],
                -1,
            )
            dboxes.append(dboxes_in_image)  # (x1, y1, x2, y2)
        return dboxes
