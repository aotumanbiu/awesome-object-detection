...
class Matcher:
    ...
    def __call__(self, match_quality_matrix: Tensor) -> Tensor:
        ...
        matched_vals, matches = match_quality_matrix.max(dim=0)  # 找到与每个先验框IoU最大的真实框 从而负责其的预测
        ...
        between_thresholds = (matched_vals >= self.low_threshold) & (matched_vals < self.high_threshold)  # 在SSD中这步不起作用
...
class SSDMatcher(Matcher):
    ...
    def __call__(self, match_quality_matrix: Tensor) -> Tensor:
        ...
        _, highest_quality_pred_foreach_gt = match_quality_matrix.max(dim=1)  # 其这一步其实是为保证每个真实框至少有一个与其相对应的先验框
