...
from anchor_utils.anchor_generator import DefaultBoxGenerator
...
class SSD(nn.Module):
    ...
    def compute_loss(
        ...
        for (
            ...
            foreground_idxs_per_image = torch.where(matched_idxs_per_image >= 0)[0]  # 正样本先验框
            foreground_matched_idxs_per_image = matched_idxs_per_image[foreground_idxs_per_image]
            num_foreground += foreground_matched_idxs_per_image.numel()

            # Calculate regression loss
            matched_gt_boxes_per_image = targets_per_image["boxes"][foreground_matched_idxs_per_image]  # 正样本先验框所对应的真实框
            bbox_regression_per_image = bbox_regression_per_image[foreground_idxs_per_image, :]  # 正样本先验框所对预测编码
            anchors_per_image = anchors_per_image[foreground_idxs_per_image, :]
            target_regression = self.box_coder.encode_single(matched_gt_boxes_per_image, anchors_per_image)  # 编码
            bbox_loss.append(
        ...
        # Calculate classification loss 计算所有的分类损失
        num_classes = cls_logits.size(-1)  # 类别说
        cls_loss = F.cross_entropy(cls_logits.view(-1, num_classes), cls_targets.view(-1), reduction="none").view(
            cls_targets.size()
        )

        # Hard Negative Sampling
        foreground_idxs = cls_targets > 0  # 前景索引
        num_negative = self.neg_to_pos_ratio * foreground_idxs.sum(1, keepdim=True)  # 分类负样本数量
        # num_negative[num_negative < self.neg_to_pos_ratio] = self.neg_to_pos_ratio
        negative_loss = cls_loss.clone()
        negative_loss[foreground_idxs] = -float("inf")  # use -inf to detect positive values that creeped in the sample
        values, idx = negative_loss.sort(1, descending=True)  # 第二个维度降序排列
        # background_idxs = torch.logical_and(idx.sort(1)[1] < num_negative, torch.isfinite(values))
        background_idxs = idx.sort(1)[1] < num_negative  # 选择topk
            ...
            if targets is None:
                ...
                for anchors_per_image, targets_per_image in zip(anchors, targets):
                    ...
                    match_quality_matrix = box_ops.box_iou(targets_per_image["boxes"],
                                                           anchors_per_image)  # 所有真实框与先验框的IoU
...
def ssd300_vgg16(
    ...
    anchor_generator = DefaultBoxGenerator(  ## 先验框超参数
        [[2], [2, 3], [2, 3], [2, 3], [2], [2]],
