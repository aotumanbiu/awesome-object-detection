...
from torchvision.models.detection._utils import overwrite_eps, _box_loss
...
def _default_anchorgen():
    ...
    aspect_ratios = ((0.5, 1.0, 2.0),) * len(anchor_sizes)  # 每个网格生成9个不同尺度的先验框
...
class RetinaNet(nn.Module):
    ...
    def compute_loss(self, targets, head_outputs, anchors):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            matched_idxs.append(
                self.proposal_matcher(match_quality_matrix))  # self.proposal_matcher 真实框和先验框匹配 返回每个先验负责真实框的索引
    ...
    def forward(self, images, targets=None):
        ...
        # Check for degenerate boxes 检查是否有标注错误的真实框
        ...
        features = self.backbone(images.tensors)  # FPN输出结果
